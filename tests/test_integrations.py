...
from mnemoth import integrations
...
def test_install_and_uninstall_every_host(tmp_path, monkeypatch):
    ...
    for host in integrations.HOSTS:
        res = integrations.install(host, command=["uvx", "mnemoth", "serve"])
    ...
    assert 'model = "gpt-5"' in toml and "[mcp_servers.other]" in toml and "[mcp_servers.mnemoth]" in toml
    claude = json.loads((tmp_path / ".claude.json").read_text())
    assert claude["theme"] == "dark" and set(claude["mcpServers"]) == {"other", "mnemoth"}
    oc = json.loads((tmp_path / ".config/opencode/opencode.jsonc").read_text())
    assert oc["mcp"]["mnemoth"] == {"type": "local", "command": ["uvx", "mnemoth", "serve"], "enabled": True}
    ...
    toml = (tmp_path / ".codex" / "config.toml").read_text()
    assert "[mcp_servers.other]" in toml and "mnemoth" not in toml
