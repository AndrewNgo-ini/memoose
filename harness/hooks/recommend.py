...
def build_guidance(project, user) -> str:
    """What memory says comes next, keyed on the Position the agent declared rather than on the prompt.

    A recall keyed on the user's words finds facts *about* things. A step the agent is in the
    middle of is a different question: given where it is, what does memory say to do next, under
    which condition, and what went wrong here before? That is answered by reading the outgoing
    Transitions two hops out from the declared Position, grouped by hop, the way Procedural Graphs
    (Lu et al. 2026) serialise a local subgraph. Their ablation is why this is a *local* subgraph
    and not the whole store: injecting everything lowered task success on the embodied benchmark;
    the two-hop neighbourhood raised it. No declared Position means no guidance (ADR 0005).
    """
    for conn in (project, user):
        if conn is None:
            continue
        node = current_position(conn)
        if node is None:
            continue
        edges = procedure_subgraph(conn, node["id"])
        if not edges:
            continue
        lines = [f"You are at `{node['name']}` (your declared Position). Memory holds transitions from here:"]
...
def main() -> int:
    ...
    if project is None and user is None:
        return 0
    try:
        guidance = build_guidance(project, user)
