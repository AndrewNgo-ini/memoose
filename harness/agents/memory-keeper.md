...
**Never write Procedures or Transitions.** A wrong transition steers the agent's future work, which is
worse than a wrong fact. Procedures are written by the foreground model that ran the steps and saw
the outcome (ADR 0005). If the material is about what to do next, say so in your one line and stop.
